...
public static class InputSetup
{
    ...
    public static void Deploy(AppConfig cfg, IProgress<string>? log = null)
    {
        ...
        {
            var instance = cfg.Players.ElementAtOrDefault(i)?.Index ?? i;
            DeployInstance(cfg, instance, pads, log);
        }
    }

    /// <summary>Refresh input isolation for one recovered instance without touching healthy games.</summary>
    public static void DeployInstance(AppConfig cfg, int instance, IProgress<string>? log = null)
    {
        if (!NativeAssets.Ready) NativeAssets.Extract(log);
        DeployInstance(cfg, instance, ResolvePads(cfg, log), log);
    }

    private static void DeployInstance(AppConfig cfg, int i,
        IReadOnlyList<(int Index, string Guid, string Name)> pads, IProgress<string>? log)
    {
        if (!Instances.Exists(cfg, i)) return;
        var bin = Instances.Bin64(cfg, i);
        var slot = cfg.Players.FirstOrDefault(player => player.Index == i);
        var pad = slot?.Pad ?? i;
